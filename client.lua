...
function drawLevel(r, g, b, a)
	...
	if IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
		AddTextComponentSubstringPlayerName('~w~Hearing Distance: ~c~' .. voicepolice.level)
	elseif IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		AddTextComponentSubstringPlayerName('~w~Hearing Distance: ~c~' .. voicecar.level)
	else
		AddTextComponentSubstringPlayerName('~w~Hearing Distance: ~c~' .. voice.level)
	end
	EndTextCommandDisplayText(0.175, 0.96)
end
		...
		if IsControlPressed(1, Keys['H']) then
			...
			elseif IsPedInAnyVehicle(GetPlayerPed(-1), false) then
				...
				if voice.current == 1 then
					DrawMarker(15,pos.x,pos.y,pos.z - 0, 0, 0, 0, 0, 0, 0, voice.shout, voice.shout, 0, 0, 5, 5, 5, 0, true, 0, 0, 0, 0, 0)
				elseif voice.current == 2 then
					DrawMarker(15,pos.x,pos.y,pos.z - 0, 0, 0, 0, 0, 0, 0, voice.whisper, voice.whisper, 0, 0, 5, 5, 5, 0, true, 0, 0, 0, 0, 0)
				elseif voice.current == 0 then
					DrawMarker(15,pos.x,pos.y,pos.z - 0, 0, 0, 0, 0, 0, 0, voice.default, voice.default, 0, 0, 5, 5, 5, 0, true, 0, 0, 0, 0, 0)
				end
			end	
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if voicecar.current == 0 then
			voicecar.level = 'Whisper'
		elseif voicecar.current == 1 then
			voicecar.level = 'Normal'
		end	
		
		if voicepolice.current == 0 then
			voicepolice.level = 'Whisper'
		elseif voicepolice.current == 1 then
			voicepolice.level = 'Normal'
		elseif voicepolice.current == 2 then
			voicepolice.level = 'Shout'
		end	
